...
Caveman ne se contente pas d'affirmer que c'est 75 %. Caveman le **prouve**.

Le répertoire `evals/` contient un dispositif d'évaluation à trois branches qui mesure la compression réelle des tokens par rapport à un groupe de contrôle approprié — pas seulement « verbosité contre compétence », mais « concision contre compétence ». Car comparer Caveman à un Claude verbose revient à confondre la compétence avec une concision générique. C'est de la triche. Caveman ne triche pas.

Traduit avec DeepL.com (version gratuite)

```bash
# Exécuter l'évaluation (nécessite l'interface CLI de Claude)
uv run python evals/llm_run.py

# Lire les résultats (sans clé API, fonctionne hors ligne)
